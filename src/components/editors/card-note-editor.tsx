...
import { ChevronLeft, Save, Activity, Calendar } from "lucide-react";
...
const CARD_CONFIG = {
  width: 600, // 固定宽度
  minHeight: 400, // 最小高度
  maxHeight: 600, // 最大高度（超过后滚动）
  padding: 48, // 内边距
};

export function CardNoteEditor({ cardId, onClose, onLinkClick }: CardNoteEditorProps) {
  const { getCardById, updateCard, cards, loadCardContent } = useAppStore();
  ...
  return (
    <div className="absolute inset-0 bg-[#f0f0f2] z-30 flex flex-col animate-in slide-in-from-bottom-2 duration-150">
      ...
      <div className="flex-1 flex items-center justify-center overflow-y-auto p-8">
        ...
        {/* 固定大小的卡片 */}
        <div 
          className="relative bg-white shadow-2xl border border-zinc-300 rounded-sm flex flex-col transition-all overflow-hidden"
          style={{
        ...
        >
          ...
          >
            ...
            {/* 浮动工具栏 */}
            <div className="sticky top-0 z-10 pointer-events-none flex justify-center mb-2">
            ...
            <div className="flex-1 min-h-0">
              <ZentriEditor
                ...
                className="prose prose-sm max-w-none font-serif text-base leading-relaxed text-zinc-800"
